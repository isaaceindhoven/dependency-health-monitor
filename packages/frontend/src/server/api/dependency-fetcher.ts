...
const input = {
  ...
  files: ['index.js', 'lib/*.js', 'esm.mjs', 'typings/index.d.ts', 'package-support.json'],
  type: 'commonjs',
  main: './index.js',
  exports: {
    '.': {
...
export default defineEventHandler(async (event) => {
  try {
    const dependencyFetcher = new DependencyFetcher(JSON.stringify(input));
    const { packages, errors } = await dependencyFetcher.aggregate();
